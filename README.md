...
⚙️ **參數可調**：可自訂 API Key、Gemini 模型、音訊分段時長、輸出目錄等。

📑 **SRT 轉錄摘要**：轉錄完成後，會自動生成摘要報告，快速掌握關鍵事件。

📜 **完整日誌**：自動記錄執行過程並即時顯示。

🔑 **支援 Google Gemini API；也可選擇使用 Vertex AI 相容的自訂中轉端點**（例如 `https://.../api/vertex-ai`）。
...
3. 參數說明  
   ...
   *   **使用自訂端點 / Base URL**：若使用 Vertex AI 相容的中轉服務，可勾選「使用自訂端點」並填入 Base URL（例如 `https://zenmux.ai/api/vertex-ai`）。勾選後音訊會以內聯方式送出；未勾選時維持官方 Google Gemini API 的既有上傳流程。勾選但 Base URL 留空時，程式會阻止任務開始。
   ...
   *   **輸出目錄**：用於存放最終 SRT 與 SRT 轉錄情況報告；留空時會輸出到來源影音檔同一資料夾。
...
*   `[您的影片名稱].srt`：**最終的字幕成品！** 會輸出到「輸出目錄」；若留空，則輸出到來源影音檔同一資料夾。
*   `[您的影片名稱]_SRT轉錄情況_[時間].txt`：由 AI 分析日誌後生成的專注於 SRT 生成情況的摘要報告，輸出位置同上。
*   `logs/[您的影片名稱]_日誌_[時間].txt`：本次執行的完整文字日誌，固定儲存在主程式資料夾下的 `logs` 資料夾。

`temp` 資料夾仍只負責保存分割音訊、分段 SRT、raw response 等中間檔，方便恢復任務或手動檢查。
...
- [ ] **自訂端點支援補強**：目前支援 Vertex AI 相容端點與內聯音訊；後續可再評估更多中轉服務的錯誤訊息、模型命名與 API Key 管理差異。

- [ ] **GUI 佈局修正**：GUI smoke test 已發現視窗最大化與還原大小時，部分區域（例如「即時日誌」）會被遮擋，需重新整理 layout/style。
